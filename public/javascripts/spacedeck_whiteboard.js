...
function setup_whiteboard_directives() {
  ...
  // Handler class that encapsulates all whiteboard logic
  // In Vue 1, methods were on the directive object (accessible via `this`).
  // In Vue 2, we store them on a handler instance attached to the element.
  function WhiteboardHandler(el, vnode) {
    this.el = el;
    this.$scope = vnode.context.$root;
    this.mouse_state = "idle";
    this.lasso = null;
    this.space_zoom = 1;
    this.artifacts_before_transaction = [];
    this.last_mouse_move_time = 0;
    this.old_panx = 0;
    this.old_pany = 0;
    this._no_artifact_toolbar_this_round = false;
  }

  WhiteboardHandler.prototype.handle_key_down_artifact = function(evt) {
    var $scope = this.$scope;
  };

  WhiteboardHandler.prototype.handle_key_up_artifact = function(evt) {
    var $scope = this.$scope;
  };

  WhiteboardHandler.prototype.handle_mouse_down_artifact = function(evt) {
    var $scope = this.$scope;
  ...
  WhiteboardHandler.prototype.handle_double_click_artifact = function(evt) {
    var $scope = this.$scope;
  ...
  WhiteboardHandler.prototype.handle_transform_mouse_down = function(evt,origin_x,origin_y) {
    evt.stopPropagation();
    evt.preventDefault();
    var $scope = this.$scope;
  ...
  WhiteboardHandler.prototype.handle_vector_transform_mouse_down = function(evt) {
    evt.stopPropagation();
    evt.preventDefault();
  ...
  WhiteboardHandler.prototype.handle_wheel_space = function(evt) {
    var $scope = this.$scope;
  ...
  WhiteboardHandler.prototype.handle_mouse_down_space = function(evt, force) {
  ...
  WhiteboardHandler.prototype.start_pan = function(evt) {
    var $scope = this.$scope;

    var spaceEl = $("#space")[0];
    if (spaceEl) {
      this.mouse_state = "pan";
      this.old_panx = spaceEl.scrollLeft;
      this.old_pany = spaceEl.scrollTop;
    }
  ...
  WhiteboardHandler.prototype.deselect = function() {
    this.$scope.deselect();
  };

  WhiteboardHandler.prototype.select = function(evt, a) {
    this.$scope.select(evt, a);
  };

  WhiteboardHandler.prototype.multi_select = function(arts) {
    this.$scope.multi_select(arts);
  };

  WhiteboardHandler.prototype.start_lasso = function(evt) {
  ...
  WhiteboardHandler.prototype.rects_intersecting = function(r1,r2) {
  ...
  WhiteboardHandler.prototype.artifacts_in_rect = function(rect) {
    if (!rect) return [];
    ...
    return _.filter($scope.active_space_artifacts, function(a) {
      return _this.rects_intersecting(a, rect);
    });
  };

  WhiteboardHandler.prototype.abs_rect = function(rect) {
    var res = {
      x: rect.x,
      y: rect.y,
      w: Math.abs(rect.w),
  ...
  WhiteboardHandler.prototype.lasso_style = function() {
  ...
  WhiteboardHandler.prototype.render_lasso = function() {
    if (!this.lasso) {
      $("#lasso").hide();
      return;
    }
  ...
  WhiteboardHandler.prototype.cursor_point_to_space = function(evt) {
    var $scope = this.$scope;
  ...
  WhiteboardHandler.prototype.rect_to_points = function(rect) {
    return [
      {x:rect.x,y:rect.y},
  ...
  WhiteboardHandler.prototype.old_selection_rect = function() {
    var $scope = this.$scope;
  ...
  WhiteboardHandler.prototype.snap_point = function(x,y,snap_middle) {
    var $scope = this.$scope;
    var _this = this;
  ...
  WhiteboardHandler.prototype.start_drawing_note = function(evt) {
    evt.preventDefault();
    evt.stopPropagation();
  ...
  WhiteboardHandler.prototype.start_drawing_scribble = function(evt) {
    evt.preventDefault();
    evt.stopPropagation();
    ...
    $scope.update_board_artifact_viewmodel(a);
    $scope.active_space_artifacts.push(a);
  ...
  WhiteboardHandler.prototype.start_drawing_arrow = function(evt) {
  ...
  WhiteboardHandler.prototype.start_drawing_line = function(evt) {
    evt.preventDefault();
    evt.stopPropagation();
  ...
  WhiteboardHandler.prototype.snap_point_simple = function(point) {
  ...
  WhiteboardHandler.prototype.handle_mouse_up_space = function(evt) {
    var $scope = this.$scope;
  ...
  WhiteboardHandler.prototype.handle_mouse_leave = function(evt) {
    var $scope = this.$scope;
  ...
  WhiteboardHandler.prototype.handle_mouse_enter = function(evt) {
    var $scope = this.$scope;
  ...
  WhiteboardHandler.prototype.handle_mouse_move = function(evt) {
    var $scope = this.$scope;
    ...
    if (this.mouse_state == "move") {
      ...
      $scope.update_selected_artifacts(function(a) {
        ...
        return _this.normalize_control_points(simplify_scribble_points(control_points), old_a);
      });
      ...
      spaceEl.scrollLeft -= dx*$scope.viewport_zoom;
      spaceEl.scrollTop  -= dy*$scope.viewport_zoom;

      $scope.handle_scroll();
    }
  };

  WhiteboardHandler.prototype.normalize_control_points = function(control_points, artifact) {
  ...
  // Register the Vue 2 directive
  Vue.directive('sd-whiteboard', {
    inserted: function (el, binding, vnode) {
      var handler = new WhiteboardHandler(el, vnode);
      el._sd_whiteboard = handler;

      $(el).on(edown, ".artifact", handler.handle_mouse_down_artifact.bind(handler));
      $(el).on("dblclick", ".artifact", handler.handle_double_click_artifact.bind(handler));
      $(el).on("keyup", ".artifact", handler.handle_key_up_artifact.bind(handler));
      $(el).on("keydown", ".artifact", handler.handle_key_down_artifact.bind(handler));
      $(el).bind("touchstart", handler.handle_mouse_down_space.bind(handler));
      $(el).bind("touchmove", handler.handle_mouse_move.bind(handler));
      $(el).bind("touchend", handler.handle_mouse_up_space.bind(handler));
      $(el).bind("mousedown", handler.handle_mouse_down_space.bind(handler));
      $(el).bind("mousemove", handler.handle_mouse_move.bind(handler));
      $(el).bind("mouseup", handler.handle_mouse_up_space.bind(handler));

      $(el).bind("wheel", handler.handle_wheel_space.bind(handler));

      $(document.body).bind("mouseleave", handler.handle_mouse_leave.bind(handler));
      $(document.body).bind("mouseenter", handler.handle_mouse_enter.bind(handler));

      $(el).find(".handle.resize-nw").bind(edown, function(e){handler.handle_transform_mouse_down(e,1,1)});
      $(el).find(".handle.resize-n").bind(edown, function(e){handler.handle_transform_mouse_down(e,0.5,1)});
      $(el).find(".handle.resize-ne").bind(edown, function(e){handler.handle_transform_mouse_down(e,0,1)});
      $(el).find(".handle.resize-e").bind(edown, function(e){handler.handle_transform_mouse_down(e,0,0.5)});
      $(el).find(".handle.resize-se").bind(edown, function(e){handler.handle_transform_mouse_down(e,0,0)});
      $(el).find(".handle.resize-s").bind(edown, function(e){handler.handle_transform_mouse_down(e,0.5,0)});
      $(el).find(".handle.resize-sw").bind(edown, function(e){handler.handle_transform_mouse_down(e,1,0)});
      $(el).find(".handle.resize-w").bind(edown, function(e){handler.handle_transform_mouse_down(e,1,0.5)});

      $(el).find(".edge-handle.resize-n").bind(edown, function(e){handler.handle_transform_mouse_down(e,0.5,1)});
      $(el).find(".edge-handle.resize-s").bind(edown, function(e){handler.handle_transform_mouse_down(e,0.5,0)});
      $(el).find(".edge-handle.resize-e").bind(edown, function(e){handler.handle_transform_mouse_down(e,0,0.5)});
      $(el).find(".edge-handle.resize-w").bind(edown, function(e){handler.handle_transform_mouse_down(e,1,0.5)});

      $(el).on(edown, ".vector-handle", function(e){handler.handle_vector_transform_mouse_down(e)});
    ...
    unbind: function (el) {
      $(el).off(edown+" "+emove+" "+eup+" "+"keyup keydown mouseleave dblclick wheel");
      $(document.body).unbind("mouseleave mouseenter");
      delete el._sd_whiteboard;
    }
  });
}
