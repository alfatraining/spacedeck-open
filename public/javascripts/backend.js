  ...
  req.onerror = function (err) {
    ...
    if (!window._spacedeck_location_change) {
      if (window.spacedeck && window.spacedeck.active_space) {
        ...
        alert(
          "Could not connect to Spacedeck. Please reconnect and try again."
        );
      }
    }
...
function save_artifact(a, on_success, on_error) {
  if (
    a._id &&
    a._id.indexOf(window.constants.CLIENT_ARTIFACT_ID_PREFIX) === -1
  ) {
    load_resource(
      "put",
      "/spaces/" + a.space_id + "/artifacts/" + a._id,
      a,
      on_success,
      on_error
    );
...
function bulk_delete_artifacts(spaceId, artifactIds, on_success, on_error) {
  load_resource(
    "post",
    "/spaces/" + spaceId + "/artifacts/batch-delete",
    { artifactIds },
    on_success,
    on_error
  );
...
function save_membership(space, m, on_success, on_error) {
  load_resource(
    "put",
    "/spaces/" + space._id + "/memberships/" + m._id,
    m,
    on_success,
    on_error
  );
...
function accept_invitation(id, code, on_success, on_error) {
  load_resource(
    "get",
    "/memberships/" + id + "/accept?code=" + code,
    null,
    on_success,
    on_error
  );
...
function get_join_link(space_id, on_success, on_error) {
  load_resource(
    "get",
    "/invitation_codes?space_id=" + space_id,
    null,
    on_success,
    on_error
  );
...
function create_join_link(space_id, role, on_success, on_error) {
  load_resource(
    "post",
    "/invitation_codes",
    { join_role: role, sticky: true, space_id: space_id },
    on_success,
    on_error
  );
}
...
function save_space_background_file(space, file, on_success, on_error) {
  load_resource(
    "post",
    "/spaces/" + space._id + "/background?filename=" + file.name,
    file,
    on_success,
    on_error
  );
...
function create_session_for_oauthtoken(token, on_success, on_error) {
  load_resource(
    "get",
    "/users/loginorsignupviagoogle?code=" + token,
    null,
    on_success,
    on_error
  );
...
function create_password_reset(email, on_success, on_error) {
  load_resource(
    "post",
    "/users/password_reset_requests?email=" + encodeURIComponent(email),
    null,
    on_success,
    on_error
  );
...
function confirm_password_reset(password, confirm, on_success, on_error) {
  load_resource(
    "post",
    "/users/password_reset_requests/" + confirm + "/confirm",
    { password: password },
    on_success,
    on_error
  );
...
function update_comment(space_id, data, on_success, on_error) {
  load_resource(
    "post",
    "/spaces/" + space_id + "/messages/" + data._id,
    data,
    on_success,
    on_error
  );
